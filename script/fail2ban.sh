...
mkdir -p /root/SecureMyServer/sources/${FAIL2BAN_VERSION}/ >>"${main_log}" 2>>"${err_log}"
cd /root/SecureMyServer/sources/${FAIL2BAN_VERSION}/ >>"${main_log}" 2>>"${err_log}"
...
cp /root/SecureMyServer/configs/fail2ban/jail.local /etc/fail2ban/jail.local
...
rm -R /root/SecureMyServer/sources/${FAIL2BAN_VERSION}
}